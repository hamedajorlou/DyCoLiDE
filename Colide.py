...
class colide_nv_batch:
    ...
    def _update_sigma_online(self, x, W):
        """
        Update sigma using exponential moving average of squared residuals.
        Uses EMA to avoid explosion from cumulative accumulation in SGD.
        """
        residual = x - W.T @ x  # (d,)
        epsilon = residual ** 2  # (d,) squared residual per node
        self.e_sigma = self.ema_alpha * self.e_sigma + (1 - self.ema_alpha) * epsilon
        sigma = np.maximum(np.sqrt(self.e_sigma), self.sigma_0)
        return sigma

    def _update_sigma_batch(self, X_batch, W):
        """
        Update sigma using exponential moving average of squared residuals.
        Uses EMA to avoid explosion from cumulative accumulation in SGD.
        """
        residuals = X_batch - X_batch @ W  # (n_batch, d)  Note: X @ W, not X @ W.T
        epsilon = np.mean(residuals ** 2, axis=0)  # (d,) avg squared residual per node
        self.e_sigma = self.ema_alpha * self.e_sigma + (1 - self.ema_alpha) * epsilon
        sigma = np.maximum(np.sqrt(self.e_sigma), self.sigma_0)
        return sigma
    ...
    def minimize_batch(self, W, sigma, mu, n_batches, batch_size, s, lr,
        ...
        NV sigma is updated from residual sufficient statistics (Appendix B.2).
        """
        obj_prev = 1e16
        self.opt_m, self.opt_v = 0, 0
        ...
        for batch_idx in range(1, n_batches + 1):
            ...
            if self.online_mode:
                ...
                # self._update_covariance_online(x, t)
                # self.cov_batch = self.cov

            else:
                # Mini batch
                batch_indices = np.random.choice(indices, size=batch_size, replace=False)
                X_batch_raw = self.X[batch_indices]  # Keep raw for sigma
                X_batch = X_batch_raw - X_batch_raw.mean(axis=0, keepdims=True)

                # Update covariance estimate (uses centered data)
                self._update_covariance(X_batch, batch_idx)
                self.cov_batch = self.cov

            # Feasibility for log det term
            M = sla.inv(s * self.Id - W * W) + 1e-16
            while np.any(M < -1e-6):
                if batch_idx == 1 or s <= 0.9:
                    return W, sigma, False
            ...
            # Gradient for W using current sigma
            inv_SigMa = np.diag(1.0 / sigma)
            ...
            # Update sigma AFTER W update (uses new W for residuals)
            if self.online_mode:
                sigma = self._update_sigma_online(x, W)
            else:
                sigma = self._update_sigma_batch(X_batch_raw, W)  # Use raw data
    ...
    def fit(self, X, lambda1, T=5,
        ...
        # Lower bound sigma0 as in Appendix B.2
        self.sigma_0 = np.sqrt(np.diag(X.T @ X / self.n)) * 1e-2

        # EMA for sufficient statistics (avoids explosion in SGD)
        self.ema_alpha = ema_alpha
        # Initialize e_sigma from data variance as starting point
        self.e_sigma = np.diag(X.T @ X / self.n).astype(self.dtype)
        ...
        total_batches = (T - 1) * n_batches_warm + n_batches_final
        mode_desc = "DyCoLiDE NV online" if batch_size == 1 else f"DyCoLiDE NV bs={batch_size}"
...
#===================================#
#  NV Batch with Covariance-based   #
#  Sigma Update (like EV-batch)     #
#===================================#

class colide_nv_batch_cov:
    """
    CoLiDE-NV with mini-batch stochastic gradient descent.
    Uses covariance-based sigma update (similar to colide_ev_batch).

    Key difference from colide_nv_batch:
    - Instead of EMA-based sigma updates, this uses the cumulative
      covariance estimate to compute per-node variances.
