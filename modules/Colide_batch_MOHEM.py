...
class colide_ev_batch:
    ...
    def _update_sigma_sufficient_stat(self, W, X_batch):
        """
        Update sufficient statistics for sigma estimation.
        residual = 1/(n_batch * d) * ||X_batch - W^T X_batch||_F^2
        e_t = e_{t-1} + residual
        sigma_t = sqrt(e_t / t)
        """
        n_batch = X_batch.shape[0]
        residual = np.linalg.norm(X_batch - X_batch @ W.T, 'fro')**2 / (n_batch * self.d)
        return residual
...
class colide_nv_batch:
    ...
    def _update_sigma_sufficient_stats(self, W, X_batch, t):
        """
        Update sufficient statistics for each node's sigma.
        For node j:
            epsilon_{j,t} = ||(x_t)_j - (W^T x_t)_j||^2
            e_{j,t} = e_{j,t-1} + epsilon_{j,t}
            sigma_{j,t} = sqrt(e_{j,t} / t)
        """
        n_batch = X_batch.shape[0]
        residuals = X_batch - X_batch @ W.T  # (n_batch, d)

        # Compute residual for each node
        for j in range(self.d):
            epsilon_j = np.sum(residuals[:, j]**2) / n_batch
            self.e_sigma[j] = self.e_sigma[j] + epsilon_j

        # Update sigma for each node
        sigma = np.sqrt(self.e_sigma / t)
        return sigma
    ...
    def minimize_batch(self, W, sigma, mu, n_batches, batch_size, s, lr,
        ...
        Args:
            batch_size: Size of mini-batch (1 for online mode)
        """
        obj_prev = 1e16
        self.opt_m, self.opt_v = 0, 0

        # Sufficient statistics for each node
        self.e_sigma = np.zeros(self.d)
        ...
        for batch_idx in range(1, n_batches + 1):
            if self.online_mode:
                # Online mode: process samples sequentially
                x = self.X[self.online_sample_idx]
                self.online_sample_idx = (self.online_sample_idx + 1) % n_total
                self.online_count += 1
            ...
            sigma = np.maximum(sigma, 1e-8)  # Ensure positive
