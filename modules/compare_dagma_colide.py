"""
Comparison: DAGMA vs GOLEM vs CoLiDE vs DyCoLiDE
...
# GOLEM (PyTorch)
from golem_torch import run_golem
...
def run_golem_ev(X, lambda1=0.02, lambda2=5.0, num_iter=50000, lr=1e-3, seed=1):
    """Run GOLEM-EV (PyTorch implementation)."""
    W_est = run_golem(X, lambda1=lambda1, lambda2=lambda2,
                      equal_variances=True, num_iter=num_iter, lr=lr, seed=seed)
    return W_est


def run_colide(X, lambda1=0.10, seed=42):
...
def run_dycolide(X, lambda1=0.07, batch_size=200, seed=42):
...
def run_comparison(n_nodes=50, n_samples=1000, edges=None, seed=42, threshold=0.32):
    ...
    print("\nGenerating data (Equal Variance, var=5)...")
    ...
    # Run GOLEM
    print("\n" + "-"*80)
    print("[2/4] Running GOLEM (PyTorch)...")
    print("-"*80)
    try:
        # Adjust iterations based on graph size (matrix_exp is O(d^3))
        golem_iter = 50000 if n_nodes <= 30 else (30000 if n_nodes <= 50 else 20000)
        start = time.time()
        W_golem = run_golem_ev(X, lambda1=0.02, lambda2=5.0, num_iter=golem_iter, lr=1e-3, seed=seed)
    ...
    try:
        start = time.time()
        W_colide = run_colide(X, lambda1=0.10, seed=seed)
    ...
    try:
        start = time.time()
        W_dycolide = run_dycolide(X, lambda1=0.07, batch_size=200, seed=seed)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--threshold', type=float, default=0.32, help='Edge threshold')
